...
describe('TimerUtil', () => {
	it('should normalize mixed time units to the smallest unit with the least digits', () => {
		...
		expect(TimerUtil.normalizeTimeUnits(input)).toEqual(expected);
	});
	...
	it('should return the input unchanged if no time units are found', () => {
		...
		expect(TimerUtil.normalizeTimeUnits(input)).toEqual([input]);
	});
	...
	it('should normalize time units for a complex input array', () => {
		const input = ['ab(cd)(e-f)(g 1000ms h)(i 3s j|)', 'ab-c--e-f--g-h--i---j|', '(a 1m b)'];
		const expected = ['ab(cd)(e-f)(g 1s h)(i 3s j|)', 'ab-c--e-f--g-h--i---j|', '(a 60s b)'];
		expect(TimerUtil.normalizeTimeUnits(input)).toEqual(expected);
	});
